...
| -----------------------| ------------ | -------------| --------------------------------------------------------------- |
| [`title`](#)           | `string`     |              | Переданное значение устанавливается как заголовок компонента.   |
| [`onSubmit`](#)        | `function`   |              | Функция обработки отправления данных из компонента на сервер.   |
...
| [`value`](#)           | `string`     |              | Значение, которое отправится на сервер при его выборе.          |
| [`commentOptions`](#)  | `object`     |              | Объект настройки комментариев. Принимает `required`, `subtitle`, `placeholder`.  Если объект не передан, блок комментария не будет появляться.|
